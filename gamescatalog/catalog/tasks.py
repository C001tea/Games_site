...
from .models import Game, WishList, PriceHistory, GamePrice, Store
...
def update_prices():
    ...
    try:
            ...
            for game in games:
                ...
                for deal in prices_response.json().get('deals', []):
                    ...
                    if game_price is None or game_price.price is None:
                        ...
                        if new_price < game_price.price:
                            subscribers = WishList.objects.filter(
                                game=game,
                                is_active=True,
                            ...
                            for item in subscribers:
                                notifications[item.user.email].append({
                                    'game': game.name,
                                    'slug': game.slug,
                                    'old_price': game_price.price,
                                    'new_price': new_price,
        ...
        for email, drops in notifications.items():
            lines = []
            for drop in drops:
                lines.append(
                    f"- {drop['game']}: ${drop['old_price']} → ${drop['new_price']} on {drop['store']}\n"
                    f"  https://game-vault.dev/game/{drop['slug']}/"
                )
            message = "Price drops on your wishlist:\n\n" + "\n\n".join(lines)

            send_mail(
                subject=f"Price drops on your wishlist ({len(drops)} game{'s' if len(drops) > 1 else ''})",
                message=message,
                from_email='noreply@game-vault.dev',
                recipient_list=[email],
                fail_silently=True,
            )

        send_mail(
            subject='GameVault: prices updated successfully!',
        ...
        send_mail(
            subject='GameVault: prices update failed!',
